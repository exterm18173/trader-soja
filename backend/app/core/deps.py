...
def get_farm_id_required(
    x_farm_id: int | None = Header(default=None, alias="X-Farm-Id"),
) -> int:
    if not x_farm_id:
        raise HTTPException(status_code=400, detail="Informe X-Farm-Id no header")
    return x_farm_id
...
def get_current_farm_membership(
    farm_id: int = Depends(get_farm_id_required),
